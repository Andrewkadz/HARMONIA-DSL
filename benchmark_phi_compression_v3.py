...
# --- 3. The Full Compound Φπε Codec (V3) ---
# Implements "Atomic Decomposition" of English concepts.
# English words are broken down into their Φπε functional components.
...
PHI_CODEC_V3 = {
    ...
    # Replacing complex words with atomic operator combinations
...
def simulate_phi_compression_v3(text):
    """
    Simulates the compression of English text into Φπε code using V3 Compound Codec.
    """
    compressed_text = text.lower()
    
    # 1. Apply Operator Replacements (Longest first to avoid partial matches)
    # We sort keys by length descending to ensure "free will" matches before "free"
    sorted_patterns = sorted(PHI_CODEC_V3.keys(), key=len, reverse=True)
    
    for pattern in sorted_patterns:
        symbol = PHI_CODEC_V3[pattern]
    ...
    # In V3, we assume a continuous stream without spaces, using operators as delimiters
    compressed_text = re.sub(r"\s+", "", compressed_text).strip()
    compressed_text = re.sub(r"[,.]", "", compressed_text) # Remove punctuation
...
def run_benchmark(input_text, label="Input Text"):
    ...
    # 2. Compression
    compressed_text = simulate_phi_compression_v3(input_text)
...
if __name__ == "__main__":
    print("Running Φπε Compression Benchmark V3 (Compound Logic)...")
