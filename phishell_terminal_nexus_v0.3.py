...
# ROUTED SHELL MODE — reads .hrm modules via Nexus
    ...
    for line in nexus_lines:
        if "→" in line:
            ...
            if cmd_pattern in command:
                # If matched command is in ROUTE_MAP, check if it's guide-like
                if cmd_pattern in ["guide", "library", "show symbols", "boot status", "active seal"]:
                    if os.path.exists(route_target):
                        ...
                        return f"{route_target} not found."
                else:
...
print("ΦShell-Terminal v0.3 — Nexus-Linked CLI")
print("All commands are routed through NEXUS.hrm")
print("Type 'exit' to quit. Use 'guide' to view terminal instructions.")
