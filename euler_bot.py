...
class EulerBot:
    ...
    def calculate_signals(self, df):
        """
        Generates signals based on the 263/97 Harmonic.
        """
        ...
        for i in range(EULER_CYCLE, len(df)):
            ...
            # Logic: Crossover
            if prev_price < prev_ma_97 and price > ma_97:
                df.loc[i, 'signal'] = 1 # BUY (Breakout)
            elif prev_price > prev_ma_97 and price < ma_97:
    ...
    def get_current_status(self, df):
        ...
        print("\n--- HARMONIC ANALYSIS ---")
        if price > ma_97:
            print("[YES] Price is ABOVE Gravity Wave (Bullish)")
        else:
            print("[NO]  Price is BELOW Gravity Wave (Bearish)")
...
if __name__ == "__main__":
    ...
    # Analyze BTC
    print("\n--------------------------------")
    print("RUNNING EULER BOT ON BITCOIN (BTC-USD)")
    print("--------------------------------")
    ...
    # Analyze SPX
    print("\n--------------------------------")
    print("RUNNING EULER BOT ON S&P 500 (^GSPC)")
    print("--------------------------------")
    bot_spx = EulerBot()
